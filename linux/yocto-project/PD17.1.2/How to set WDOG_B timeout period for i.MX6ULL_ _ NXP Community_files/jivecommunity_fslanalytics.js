...
var wa_contentType = ''; //Used to set Adobe Analytics variable s.prop1
...
if(typeof getAdobeCookie == "function" && typeof setAdobeCookie == "function"){
	if(getAdobeCookie("jive.cookie.consent.accepted") == "true"){
		trackEnabled = true;
	...
	if (trackEnabled === true) {
		(function () {
			function async_load() {
				...
				s.async = true;
				s.src = '//www.nxp.com/resources/scripts/analytics/elqCfg.min.js';
...
if(trackEnabled === true){
	...
	//document.write("<script src='//www.nxp.com/files/js/elqNow/elqwebanalytics.js'></script>"); //Used to set Eloqua Variable for historical data purposes s.eVar38 in Adobe Tag Manager Code

	//For getting Freescale Visitor Id
	//document.write("<script src='//www.nxp.com/webapp/visitorIdLookup.jsp'></script>"); //Used to set Adobe Analytics variable s.eVar15
	//document.write("<script src='//www.nxp.com/resources/scripts/analytics/blueconic.js'></script>");
	var s = document.createElement("script");
	s.type = "text/javascript";
	s.src = "//www.nxp.com/webapp/visitorIdLookup.jsp";
	$("head").append(s);   

	//eloqua scripts
	/*document.write("<script src='//www.nxp.com/files/js/analytics/elqCfg.js'></script>");
	document.write("<script src='//www.nxp.com/files/js/analytics/elqFCS.js'></script>");*/
}
...
if(trackEnabled === true){
	...
	searchTerm = '';
	contentId = '';
	currentSpace = jQuery('.j-menu.j-breadcrumb-popover ul li a.last-child').text(); //No Updates Required
	parentSpace = jQuery('.j-menu.j-breadcrumb-popover ul ul li a:not(.last-child)').text(); //No updates Required
	...
	if(jQuery('#j-search-input').length>0){
		searchTerm = jQuery('#j-search-input').val();
	}

	if(jQuery('header#j-compact-header img.jive-avatar').length > 0){
		userid = 0;
		username = jQuery('header#j-compact-header img.jive-avatar').attr('data-username');
	}else{
		userid = 0;
		username = "Guest";
	}

	if(document.title == "Error | Freescale Community"){
		isErrorPage = true;
	}else{
	...
	if((jQuery('nav#j-placeNav').length != 0 || jQuery('nav ul.j-tabbar').length != 0) && jQuery('#j-globalNav .active').length <= 0){   // PreCloud Migration - if(jQuery('nav.j-bigtab-nav').length != 0 && jQuery('#j-globalNav .active').length <= 0){
		title = jQuery('header.j-page-header h1 a.j-placeName').text(); // PreCloud Migration - title = jQuery('header.j-page-header h1 a.font-color-normal').text();
		// PreCloud Migration - tab = jQuery('ul.j-tabbar li.j-tab-selected').text();
		if(jQuery('ul#j-placeMainNav li.j-tab-selected').length > 0){
			tab = jQuery('ul#j-placeMainNav li.j-tab-selected').text();
		}else if(jQuery('ul.j-tabbar li.j-tab-selected').length>0){
			tab = jQuery('ul.j-tabbar li.j-tab-selected').text().trim();
		}
		...
		if(typeof _elqQ == "object"){
			eventType = "pageLoad";
			formElqUrl(title,parentSpace,tab,contentId,'',currentSpace,'','',activityType,searchTerm,tid,userid,username,eventType);
		}
	}else{
		if((jQuery('#jive-body-main').length > 0 || jQuery('#jive-body').length>0) && jQuery('#j-globalNav .active').length <= 0 && jQuery('.jive-create-large').length <=0){
			title = jQuery('.jive-content header h1').text(); //No updates Required
			wa_contentType = jQuery('#jive-body-intro .j-context a').first().text(); //No Updates Required
			jQuery('span.jive-thread-post-details-tags a span').each(function(){ // PreCloud Migration - tags = jQuery('span.jive-thread-post-details-tags a').map(function() { return this.text; }).get().join(',');
			...
			category = jQuery("span.jive-content-footer-categories:not(.jive-content-footer-tags,.jive-vid-tags) span:not('.jive-video-views-count')").text(); //No Updates Required

			title = title.replace(/\s+/g, ' ');
			tags = tags.replace(/\s+/g, '');
			category = category.replace(/\s+/g, ' ').replace('Categories:','');
			category = category.replace(/,/g, ';');
			wa_contentType = wa_contentType.replace(/\s+/g, ' ');
			wa_contentType = wa_contentType.replace('More ','');

			if(typeof _elqQ == "object"){
				eventType = "pageLoad";
				formElqUrl(title,parentSpace,tab,contentId,wa_contentType,currentSpace,category,tags,activityType,searchTerm,tid,userid,username,eventType);
			}
		}else if(jQuery('#j-globalNav .active').length > 0){ //No Updates Required
			var menuItem  = jQuery('#j-globalNav .active').attr('id'); //No Updates Required
	...
	jQuery('.acclaim-like-container').click(function(event){
		activityType = jQuery(this).find('.jive-acclaim-likelink').first().attr('data-command');
		...
		if(jQuerythis.attr('data-command') == 'showAcclaim') //Pre Cloud Migration - if(jQuerythis.attr('data-command') == 'showLikes')
			return;

		if(typeof _elqQ == "object"){
			formElqUrl(title,parentSpace,tab,contentId,wa_contentType,currentSpace,category,tags,activityType,searchTerm,tid,userid,username);
		}
		// PreCloud Migration - Webtrends Removal jQuery('meta[id=community_activity_type]').attr('content', activityType);
		// PreCloud Migration - Webtrends Removal jQuery.getScript( "/themes/freescale_theme_v2_0/scripts/FSL_COMM_SMARTSOURCE_TAG.js" );

		trackCommunityActivity(activityType);
	});

	jQuery('.j-js-follow-controls').click(function(){ //No updates Required
		setTimeout(function(){
			if(jQuery('#follow-in-menu').length != 0){
				jQuery('.j-stop-following').click(function(){
		...
		if(jQuery(this).find('a[idjQuery="-startFollowing"]').css("display") != "none"){
	...
	jQuery('#jive-link-thread-stopFollowing').click(function(){
		activityType = "stoppedFollowing";
		if(typeof _elqQ == "object"){
			formElqUrl(title,parentSpace,tab,contentId,wa_contentType,currentSpace,category,tags,activityType,searchTerm,tid,userid,username);
		}
	...
	jQuery('.js-link-delete').click(function(){
		activityType = "delete";
		if(typeof _elqQ == "object"){
			formElqUrl(title,parentSpace,tab,contentId,wa_contentType,currentSpace,category,tags,activityType,searchTerm,tid,userid,username);
		}
	...
	jQuery('#jive-link-thread-startTracking').click(function(){
	...
	jQuery('#jive-link-thread-stopTracking').click(function(){
	...
	jQuery('.discussionAdd').click(function(){		
		jQuery("body").delegate(".jive-form-button-save", "click", function(){
			var elem = jQuery(this).parents('#postform');
	...
	jQuery('div.j-contained-tabs-place.j-browse-content div.j-sidebar-left nav.j-second-nav-categories li').click(function(){
		category = jQuery(this).find('.nav-label').text();
		if(typeof _elqQ == "object"){
			formElqUrl(title,parentSpace,tab,contentId,wa_contentType,currentSpace,category,tags,activityType,searchTerm,tid,userid,username);
		}
	});
	...
	if (wa_contentType == ''){
		wa_contentType = "General";
	}

	siteSection = '';
	jQuery('#js-place-parents-container ul ul li a:not(.last-child)').each(function(){ // PreCloud Migration - jQuery('.j-menu.j-breadcrumb-popover ul ul li a:not(.last-child)').each(function(){
		var txt = jQuery(this).text().trim(); //No Updates Required
		if(txt != ''){
	...
	jQuery.getScript( "//assets.adobedtm.com/f2bd0c145f764c0e7b6dae5f9269061e38fabacd/satelliteLib-d232b2f7660791e89253bc84d8dac8aaf345cfbe.js", function(){

		if(typeof getFslVisitorIDForAnalytics == 'function'){				
			s_vid = getFslVisitorIDForAnalytics();
		}

		if(jQuery('#j-search-input').length>0){ //No Updates Required
			s_log = 'in search';
			title = "Search";
			channel = "Search";
			wa_contentType = "Search";
		}else{
			...
			if(currentSpace == ""){
				...
				if(pageURL.indexOf(strKey) > -1){
					if(pageURL.indexOf("/tags#/") > -1){
						var pageHead = jQuery("header.j-page-header > h1").first().text().trim(); // Pre Cloud Migration var pageHead = jQuery("header.j-page-header > h1 > a").first().text();
						pageHead = jQuery.trim(pageHead.replace("Browse all tags in",""));
						if(jQuery("header.j-page-header > h1 > span.j-page-header-parent").length > 0){	//if(jQuery("header.j-page-header > h1 > span.j-page-header-parent > a").length > 0){
							parentSpace = jQuery("header.j-page-header > h1 > span.j-page-header-parent").first().text().trim(); //parentSpace = jQuery("header.j-page-header > h1 > span.j-page-header-parent > a").first().text();
							currentSpace=jQuery.trim(pageHead.replace(parentSpace,""));
							parentSpace = jQuery.trim(parentSpace.replace("in ",""));
						}
						else{
		...
		if(isErrorPage){
			parentSpace = "";
			wa_contentType = "Error";
			channel = "Error";
			title = "Error";
			if(jQuery("div#jive-body > div#jive-body-intro > h1").length > 0){  //No updates required
				contentId = wa_contentType + ":" + jQuery("div#jive-body > div#jive-body-intro > h1").first().text(); //No updates required
				title = jQuery("div#jive-body > div#jive-body-intro > h1").first().text(); //No updates required
			}
		...
		if(channel == "Search"){
			if(jQuery('dd').length > 0){
				if(jQuery('dd.j-community-bridged').hasClass('j-active')){
			...
			if(channel == "Search"){
				...
				digitalData.pageInfo.pageType = "Community: "+wa_contentType;
	...
	jQuery('#campaignId').click(function(){
...
function formElqUrl(title,parentSpace,tab,contentId,wa_contentType,currentSpace,category,tags,activityType,searchTerm,tid,userid,username,eventType){
	if(trackEnabled === true){
		...
		if(eventType == "pageLoad" && typeof _elqQ == "object") {
			_elqQ.push(['elqTrackPageView', elqURL, document.referrer]);
			eventType = "";
		}
		else if(typeof _elq == "object")
			_elq.trackEvent('http://'+ elqURL,'', document.referrer);
		// Removed during the migration to the cloud
		//jQuery("#j-globalNav-bg").after("<iframe style='display:none' src="+webActivityUrl+"/>");
	}
}
...
function getContentId(){
	...
	if(contentId.indexOf("#") > -1){
		var cp = jQuery.trim(contentId).split("#");
		if(cp.length > 1){
	...
	if(pageURL.indexOf("/blog/") > -1 && pageURL.indexOf("/tags#/") == -1 && pageURL.indexOf("/blog/create-post.jspa") == -1){
		type = "blog";
		if(jQuery(".jive-content-blog").length > 0)
			contentId = jQuery(".jive-content-blog").attr("data-object-id"); // PreCloud Migration contentId = jQuery(".jive-content-blog").attr("aria-labeledby").replace("heading-", "");
	}
...
function trim1 (str) {
	return str.replace(/^\s\s*/, '').replace(/\s\s*jQuery/, '');
} 
...
function getReffererPageType(){
	return getAdobeCookie("referrerPageType");
}
...
function getContentFindingFromReferrer(referringUrl, isDownload){	
	var reffererPageCategory = getReffererPageCategory();
	var reffererPageCode = getReffererPageCode();
	var reffererPageType = getReffererPageType();
	var pageCategory = getPageCategory();
	var pageCode = getPageCodeID(); 
	...
	if(reffererPageCategory == 'PART DETAILS' && pageCategory != 'PART DETAILS')
		return "Part Details";	
	
	if(reffererPageCategory == 'DSP' && reffererPageCode != pageCode) 
		return 'DSP';

	if(reffererPageCategory == 'TSP' && reffererPageCode != pageCode) 
		return 'TSP'; 

	if(reffererPageCategory == 'GUIDE SUMMARY' && reffererPageCode != pageCode)   
	...
	if(reffererPageCategory == 'HOMEPAGE' || reffererPageCategory == 'OVERVIEW' || reffererPageCategory == 'TAXONOMY'){
		if(reffererPageCode != pageCode && (reffererPageType == 'Family'|| pageCategory=='DOWNLOAD'&& pageType=='Family')) {
			return "Product Family";
		}else if(reffererPageCode != pageCode && (reffererPageType == 'General' || pageCategory=='DOWNLOAD'&& pageType=='General')){
			return "General";
		}
	}	
	...
	if(reffererPageCategory == 'PART DETAILS')
		return "Part Details";	
	...
	if(reffererPageCategory == 'Solutions' && reffererPageCode != pageCode)
		return "Solutions";
	
	if(reffererPageCategory == 'General' && reffererPageCode != pageCode)
		return "General";
...
function initContentFinding(){
	...
	jQuery('a').attr('data-content-finding', 'Community');

	//Footer Social Media Exit Links
	jQuery('ul#footer-social a').attr('data-content-finding','Community Footer');
	jQuery('ul#footer-social a').attr('data-content-subfinding','FOLLOW US');
	
	//Footer News Links
	jQuery('div.newsItem a').attr('data-content-finding','Community Footer');
	jQuery('div.newsItem a').attr('data-content-subfinding','NEWS');
	
	jQuery("div.dtmFooter").each(function(){		
		var title = jQuery(this).find("h4").first().text();
		title = title == "" ? "Privacy": title;
		jQuery(this).attr('data-dtmname',title);
	});
	
	jQuery("footer#j-footer > ul.dnv > li:last").children("a").each(function(){
		jQuery(this).attr("data-content-finding", "Community Footer");
	});	


	jQuery("input#autosearch").keydown(function(evnt){ //No updates Required
	...
	jQuery(document).click(function(evnt) {
		if(evnt.which == 3)
			return;

		//This event handler is to set content finding

		jQuerythis = jQuery(evnt.target);
		if(jQuerythis.prop("tagName").toLowerCase() != "a"){
			if(jQuerythis.parents("a").length > 0){
				jQuerythis = jQuerythis.parents("a").first();
			}
		}

		if(jQuerythis.prop("tagName").toLowerCase() == "a"){
			var contentFindingMethod = 'Community';
			var hrf = jQuerythis.attr("href");
			...
			if(jQuerythis.parents("div.j-search-results-main-container").length > 0 && hrf.indexOf("#facet=") != 0 || jQuerythis.parents("div.j-search-results-aside-container").length > 0 && hrf.indexOf("#facet=") != 0){ //No Updates Required
				contentFindingMethod = 'Community Search';
				setContentFinding(jQuerythis, contentFindingMethod);
				return;
			}

			if(jQuerythis.parents("li.j-simple-nav-logo,li#jive-nav-link-home,li#jive-nav-link-work,li#jive-nav-link-conx,li#jive-nav-link-places").length > 0){  //if(jQuerythis.parents("li#jive-nav-link-home,li#jive-nav-link-work,li#jive-nav-link-conx,li#jive-nav-link-places").length > 0){	//No Updates Required	
				contentFindingMethod = 'Community Header';
				setContentFinding(jQuerythis, contentFindingMethod);
				return;
			}

			if(jQuerythis.parents("div#appQuickLaunchMenu").length > 0){ // No Updates Required
				contentFindingMethod = 'Community Header';
				setContentFinding(jQuerythis, contentFindingMethod);
				return;
			}

			if(jQuerythis.parents("div#menuCreate").length > 0){
				contentFindingMethod = 'Community Header';
				setContentFinding(jQuerythis, contentFindingMethod);
				return;
			}

			if((jQuerythis.parents("div#hd").length > 0 || jQuerythis.parents("ul#j-satNav").length > 0) && jQuerythis.attr("data-content-finding") != "Community Header"){ //Pre Cloud Migration - if(jQuerythis.parents("header#j-header").length > 0 && (jQuerythis.parents("h1#logo").length > 0 || jQuerythis.parents("ul#j-satNav").length > 0))
				contentFindingMethod = 'Community Header';
				setContentFinding(jQuerythis, contentFindingMethod);
				return;
			}

			if(jQuerythis.parents("div#j-satNav-menu").length > 0 && jQuerythis.attr("data-content-finding") != "Community Header"){ //No Updates Required
				contentFindingMethod = 'Community Header';
				setContentFinding(jQuerythis, contentFindingMethod);
				return;
			}
			if(jQuerythis.parents("footer#j-footer").length > 0 && jQuerythis.parents("ul.dnv").length > 0 && jQuerythis.attr("data-content-finding") != "Community Footer"){				
				contentFindingMethod = 'Community Footer';
				setContentFinding(jQuerythis, contentFindingMethod);
				return;
			}
			if(jQuerythis.attr("data-content-finding") != "Community Footer" && jQuerythis.attr("data-content-finding") != "Community Header"){
				setContentFinding(jQuerythis, contentFindingMethod);
			}
		}
	});
...
//		return;
		...
		jQuerythis = jQuery(event.target);
		if(jQuerythis.prop("tagName").toLowerCase() != "a"){
			if(jQuerythis.parents("a").length > 0){
				jQuerythis = jQuerythis.parents("a").first();
			}
		...
		if(jQuerythis.prop("tagName").toLowerCase() == "a"){
			var linkURL = jQuerythis.attr("href");
			...
			if(linkURL != ''){
				var fullURL = jQuerythis.prop("href");
...
function initSearchTracking(){
	if(searchIn == "Freescale Community"){
		if(jQuery('div#j-main-results > ol.j-search-results > li').length == 1 && jQuery('div#j-main-results > ol.j-search-results > li.j-loading-bar').length == 1){ // No Updates Required
		...
		if(jQuery('div#j-engine-results-1000 > ol.j-search-results > li').length == 1 && jQuery('div#j-engine-results-1000 > ol.j-search-results > li.j-loading-bar').length == 1){ //No Updates Required
			//search not complete, retry
			if(setTimeSearch != 0){
				clearTimeout(setTimeSearch);
				setTimeSearch = 0;
	...
	if(jQuery("div.j-search-results-main-container").length > 0){ //no updates required
		jQuery("div.j-search-results-main-container").children("a").attr('data-content-finding', 'Community Search');
	}

	jQuery("#j-search-form").unbind('submit.analytics'); // no updates required
	jQuery("#j-search-form").bind('submit.analytics', function(){ //no updates required
		if(setTimeSearch != 0){
			clearTimeout(setTimeSearch);
			setTimeSearch = 0;
		}
	...
	jQuery(".j-facets a, .j-sub-facets a").unbind('click.analytics'); //No updates Required
	jQuery(".j-facets a, .j-sub-facets a").bind('click.analytics', function(){ //No updates Required
		if(setTimeSearch != 0){
			clearTimeout(setTimeSearch);
			setTimeSearch = 0;
		}
	...
	jQuery(".j-view-more a").unbind('click.analytics');
	jQuery(".j-view-more a").bind('click.analytics', function(){
		if(setTimeSearch != 0){
			clearTimeout(setTimeSearch);
			setTimeSearch = 0;
		}
	...
	jQuery('.jive-autocomplete-search-selected a.remove').unbind('click.analytics');
	jQuery('.jive-autocomplete-search-selected a.remove').bind('click.analytics',function(){
		if(setTimeSearch != 0){
			clearTimeout(setTimeSearch);
			setTimeSearch = 0;
		}
	...
	jQuery('input#nameonly[type="checkbox"]').unbind('click.analytics');
	jQuery('input#nameonly[type="checkbox"]').bind('click.analytics', function() {
		if(setTimeSearch != 0){
			clearTimeout(setTimeSearch);
			setTimeSearch = 0;
		}		
	...
	jQuery('a[href|="#facet="]').unbind('click.analytics');
	jQuery('a[href|="#facet="]').bind('click.analytics', function(){
		if(setTimeSearch != 0){
			clearTimeout(setTimeSearch);
			setTimeSearch = 0;
		}		
	...
	jQuery('select#sort').unbind('change.analytics');
	jQuery('select#sort').bind('change.analytics', function(){
		if(setTimeSearch != 0){
			clearTimeout(setTimeSearch);
			setTimeSearch = 0;
		}		
	...
	jQuery('input#author').unbind('focus.analytics');
	jQuery('input#author').bind('focus.analytics', function(){
		if(setIntAuth != 0){
	...
	jQuery('input#place').unbind('focus.analytics');
	jQuery('input#place').bind('focus.analytics', function(){
		if(setIntPlace != 0){
	...
	jQuery('input#author').unbind('focusout.analytics');
	jQuery('input#author').bind('focusout.analytics', function(){
		if(setIntAuth != 0){
			clearInterval(setIntAuth);
			setIntAuth = 0;
		}
	...
	jQuery('input#place').unbind('focusout.analytics');
	jQuery('input#place').bind('focusout.analytics', function(){
		if(setIntPlace != 0){
			clearInterval(setIntPlace);
			setIntPlace = 0;
		}
...
function waitForAutoComplete(){
	jQueryautoComplete = jQuery('div.j-autocomplete li.user-autocomplete-item').find('a');
	if(jQueryautoComplete.length > 0){
		jQueryautoComplete.unbind('click.analytics');
		jQueryautoComplete.bind('click.analytics', function(){
...
function trackSearchForAnalytics(){
	if(trackEnabled === true){
		...
		if(searchIn == "Freescale Community"){
			if(jQuery("#j-main-results .j-no-results").length > 0){	
				searchResultCount = "zero";
			}
		}else{
			if(jQuery("#j-engine-results-1000 .j-no-results").length > 0){	
				searchResultCount = "zero";
			}
		}
...
function getFilterFromQueryString(){	
			...
			for(i=0;i<queryParams.length;i++){
				...
				if(queryParam.length > 1){
					...
					if(excludeParams.indexOf(';' + paramName + ';') == -1 && paramValue != ''){
						filterString += 'jQuery' + paramName + '~' + paramValue;
						if(paramName == 'facet'){
							defaultFacet = false;
						}
					}
	...
	if(defaultFacet){
		filterString = "jQueryfacet~content" + filterString;
	}
...
function getDownloadType(linkURL, fullURL){
	...
	var commDomains = ['//community-uat.freescale.com', '//community.freescale.com', '//freescale.jiveon.com','//community.nxp.com'];  // Pre Cloud Migration - var commDomains = ['//community-uat.freescale.com', '//community.freescale.com'];
	...
	var nonsecfslIncludeList = ['/files/', '/lgfiles/', '/mcds/', '/zh-Hans/files/', '/ja/files/', '.pdf', '.doc', '.zip', '.ppt', '.txt', '.iso', '.tar','.xls'];

	//List of file extensions that should be treated as downloads
	var nonsecIncludeList = ['.pdf', '.doc', '.zip', '.ppt', '.txt', '.iso', '.tar','.xls'];
...
function trackDwnLoad(linkURL, downloadType, contentFindingMethod){
	if(trackEnabled === true){
		...
		if (typeof s_account != 'undefined') {
			...
			hrefUrl = hrefUrl.replace('cache.freescale.com', 'www.nxp.com');
			hrefUrl = hrefUrl.replace('www.freescale.com','www.nxp.com');
			hrefUrl = hrefUrl.replace('cache.nxp.com','www.nxp.com');	
...
function getAdobeCookie(cname) {
    var name;
	if(cname != "jive.cookie.consent.accepted"){
		name = "_sc_" + cname + "=";
	}else{
		name = cname + "=";
	}
